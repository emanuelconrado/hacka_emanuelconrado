...
ManagerNode::ManagerNode(const rclcpp::NodeOptions &options) : rclcpp_lifecycle::LifecycleNode("manager_node", "", options) {
  ...
  declare_parameter("waypoints.points", rclcpp::ParameterValue());
}
...
void ManagerNode::tmrOpGoto(){
  ...
  auto request = std::make_shared<std_msgs::srv::Bool::Request>();
  auto callback_result = [this](rclcpp::Client<std_msgs::srv::Bool> SharedFuture future) -> void {
  	RCLCPP_INFO(this->get_logger(), future.get()->message.c_str));
